...
use crate::context::internal::Isolate;
use crate::context::{CallbackInfo, Context, Lock};
...
impl<'a, T: Class> ClassDescriptor<'a, T> {
    ...
    pub fn new<'b, U: Class>(
        name: &'b str,
        allocate: AllocateCallback<U>,
    ) -> ClassDescriptor<'b, U> {
        ClassDescriptor {
            name: name,
            allocate: allocate,
    ...
    fn create<'a, C: Context<'a>>(cx: &mut C) -> NeonResult<ClassMetadata> {
        ...
        unsafe {
            ...
            for (name, method) in descriptor.methods {
                let method: Handle<'_, JsValue> = build(|out| {
...
impl<T: Class> ValueInternal for T {
    fn name() -> String {
        let mut isolate: Isolate = unsafe { mem::transmute(neon_runtime::call::current_isolate()) };
