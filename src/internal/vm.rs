...
impl IsolateInternal for Isolate {
    ...
    fn class_map(&mut self) -> &mut ClassMap {
        ...
        if ptr.is_null() {
            ...
            let free_map: *mut c_void = unsafe { mem::transmute(drop_class_map as extern "C" fn(_)) };
            unsafe {
...
impl<'a> Module<'a> {
    pub fn initialize(exports: Handle<JsObject>, init: fn(Module) -> VmResult<()>) {
        ...
        unsafe {
            ...
            let callback: extern "C" fn(*mut c_void, *mut c_void, *mut c_void) = mem::transmute(module_callback as extern "C" fn(_,_,_));
