...
fn main() {
    ...
    // Detect filename collisions in sanitize_module_name (e.g., `foo-bar.md`
    // and `foo_bar.md` both collapse to `doc_foo_bar`). The naïve sanitizer
    // would silently emit two `pub mod doc_foo_bar {}` and break the build
    // with a confusing E0428; surface the real cause instead.
    let mut seen: HashMap<String, PathBuf> = HashMap::new();
    for path in &entries {
        let rel = path.strip_prefix(&docs_root).expect("path under docs_root");
        let module_name = sanitize_module_name(rel);
        if let Some(prev) = seen.insert(module_name.clone(), path.clone()) {
            panic!(
                "doctest module name collision: {} and {} both sanitize to `{}`. \
                 Rename one of the files so they differ in alphanumeric characters.",
                prev.display(),
                path.display(),
                module_name
            );
