...
function hiddenRoutes() {
  ...
  for (const entry of readdirSync(DOCS_ROOT, { recursive: true, withFileTypes: true })) {
    if (!entry.isFile() || !/\.mdx?$/.test(entry.name)) continue;
    const path = join(entry.parentPath, entry.name);
    const frontmatter = readFileSync(path, "utf8").match(/^---\n(.*?)\n---\n/s);
    if (!frontmatter || !/^status: (draft|todo)$/m.test(frontmatter[1])) continue;
...
export function remarkUnlinkHiddenPages() {
  ...
  const isHidden = (url) =>
    url.startsWith("/") && hidden.has(url.replace(/[#?].*$/, "").replace(/\/?$/, "/"));
