---
title: Quickstart
description: Produce a runnable hello-world Neon addon in just a few minutes.
---
...
is converted back, and the function is registered with the addon so
...
Create `example.cjs` next to `package.json`:
...
- [`#[neon::export]`](/api/neon/attr.export.html) registered `add` with
  the addon's init code, generated a wrapper that pulls each argument
