//! Facilities for working with `FunctionCallbackInfo` and getting the current `Isolate`.
...
extern "system" {
    ...
    /// Mutates the `out` argument provided to refer to the `Local` handle value of the object the
    /// function is bound to.
    #[link_name = "NeonSys_Call_This"]
    pub fn this(info: &FunctionCallbackInfo, out: &mut Local);

    /// Mutates the `out` argument provided to refer to the `Local` handle value of the currently
    /// executing function.
    #[link_name = "NeonSys_Call_Callee"]
    pub fn callee(info: &FunctionCallbackInfo, out: &mut Local);

    /// Mutates the `out` argument provided to refer to the `Local` handle value of the
    ...
    /// Mutates the `out` argument provided to refer to the `Local` handle value of the `i`th
