...
extern "C" {
    ...
    pub fn new(out: &Persistent, isolate: *mut Isolate);
    ...
    pub fn get_string(out: &Persistent, isolate: *mut Isolate, object: &Persistent, key: *const u8, len: i32) -> bool;
    ...
    pub fn set_string(out: &mut bool, isolate: *mut Isolate, object: &Persistent, key: *const u8, len: i32, val: &Persistent) -> bool;
    ...
    pub fn get(out: &Persistent, isolate: *mut Isolate, object: &Persistent, key: &Persistent) -> bool;

    /// Sets the key value of a `v8::Object` at the `key` provided. Also mutates the `out` argument
    /// provided to refer to a `v8::Local` boolean value, `true` if the set was successful.
    #[link_name = "Neon_Object_Set"]
    pub fn set(out: &mut bool, isolate: *mut Isolate, object: &Persistent, key: &Persistent, val: &Persistent) -> bool;
