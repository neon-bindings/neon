...
function Bindings(opts) {
  ...
  this._flatName = this.name.replace('-', '_');
...
Bindings.prototype.getStaticLibPath = function getStaticLibPath() {
  ...
  return "target/" + this.mode + "/lib" + this._flatName + ".a";
};

Bindings.prototype.getDynamicLibPath = function getDynamicLibPath() {
  // FIXME: this is OSX-specific
  return "target/" + this.mode + "/lib" + this._flatName + ".dylib";
};
...
Bindings.prototype.getAddonPath = function getAddonPath() {
  return path.resolve(this.root, "build/" + this.mode[0].toUpperCase() + this.mode.substring(1) + "/" + this._flatName + ".node");
};
