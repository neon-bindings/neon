...
| # | Slug | Bucket | Goal | Notes |
|---|---|---|---|---|
| 20 | `how-to/async-fn.md` | How-to | Reader can export an `async fn` that returns a Promise. | Both auto-init Tokio and explicit `set_global_executor`. Pairs with *async-tokio* tutorial; this page is the no-narrative recipe. **Home of form 1 of the async trio (`async fn`).** |
| 21 | `how-to/sync-setup-async.md` | How-to | Reader can do main-thread setup before async work, returning `impl Future` from an `#[neon::export(async)]` fn. | Already has a real code sample inherited from the homepage. Build the page around it. **Home of form 2 of the async trio (`(async) fn -> impl Future`).** Use the canonical motivator: rooting a JS callback or grabbing a `Channel` before spawning, so the future can call back into JS while running. |
| 22 | `how-to/main-thread-after-async.md` | How-to | Reader can hop back to the main thread with `extract::with` to build a JS-shaped result. | Cross-link the previous two pages. **Home of form 3 of the async trio (`extract::with`).** Show building a JS object with computed keys (e.g. column-keyed result rows) — the case where built-in `TryIntoJs` impls aren't enough. |
